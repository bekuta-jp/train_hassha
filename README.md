...
```json
{
  ...
  "published_site_url": "https://bekuta-jp.github.io/train_hassha/"
}
```

この設定はデスクトップ版の初期表示、Web 版の公開ページの初期表示、ローカル版の公開版比較先 URL に使われます。

## 更新履歴

### ver1.1 - 2026-04-15

- ローカル版が公開中の Web 版ダイヤと比較して、古い場合に更新案内を出すように改善
- Web 版の公開データにダイヤハッシュと公開状態 JSON を追加
- GitHub Actions で公式サイトを日次確認し、差分があるときだけ Web 版を自動更新する運用に対応
...
1. 公式サイトから時刻表を取得
2. 公開中の Web 版とダイヤハッシュを比較
3. 差分がある場合だけ公開用の静的 Web サイトを生成
4. GitHub Pages へデプロイ

`workflow_dispatch` と `main` への push では毎回デプロイし、`schedule` では 1 日 1 回だけ公式サイトを確認します。既定の実行時刻は `JST 04:17` 相当です。

## 更新連携の仕組み

- ローカル版は起動時と定期的に、公開中の Web 版ダイヤ状態 JSON を確認します
- 公開版の方が新しい場合は、公式サイトから再取得して保存するかどうかを案内します
- Web 版は GitHub Actions が公式サイトを日次確認し、差分があるときだけ自動更新します
- 公開比較に使う JSON は `site/assets/data/port_liner_status.json` として書き出されます
